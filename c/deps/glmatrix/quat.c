...
quat_t quat_create(quat_t quat) {
    quat_t dest = calloc(sizeof(GLMfloat), 4);
...
quat_t quat_calculateW(quat_t quat, quat_t dest) {
    GLMfloat x = quat[0], y = quat[1], z = quat[2];
...
quat_t quat_inverse(quat_t quat, quat_t dest) {
    GLMfloat dot = quat_dot(quat,quat),
        invDot = 1.0/dot;
    if(!dest || quat == dest) {
        quat[0] *= -invDot;
        quat[1] *= -invDot;
...
quat_t quat_multiply(quat_t quat, quat_t quat2, quat_t dest) {
    ...
    GLMfloat qax = quat[0], qay = quat[1], qaz = quat[2], qaw = quat[3],
...
quat_t quat_slerp(quat_t quat, quat_t quat2, GLMfloat slerp, quat_t dest) {
    ...
    GLMfloat cosHalfTheta = quat[0] * quat2[0] + quat[1] * quat2[1] + quat[2] * quat2[2] + quat[3] * quat2[3],
        halfTheta,
        sinHalfTheta,
        ratioA,
        ratioB;
